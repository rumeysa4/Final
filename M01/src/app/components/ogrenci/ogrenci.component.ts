...
OgrenciListele(){
  this.apiServis.OgreciListe().subscribe((d: Ogrenci[]) => {
   this.ogrenciler = d;
...
Filtrele(e:any){
var deger=e.target.valur;
this.dataSource.filter = deger.trim().toLowerCase();
if (this.dataSource.parginator){
...
Ekle(){
  ...
  this.dialogRef.afterClosed().subscribe(d=>{
    if(d){
    this.apiServis.OgrenciEkle(d).subscribe((s: any) => {
...
Duzenle(kayit:Ogrenci) {
  ...
  this.dialogRef.afterClosed().subscribe(d=>{
    ...
    this.apiServis.OgrenciDuzenle(kayit).subscribe((s:any) =>{
        this.alert.AlertUygula(s);
    });
...
this.confirmDialogRef.afterClosed().subscribe(d=> {
  if(d){
this.apiServis.OgrenciSil(kayit.ogrId).subscribe((s:any)=>{
  this.alert.AlertUygula(s);
  if(s.islem) {
    this.OgrenciListele();
  }
